...
from ._find_well_engine import FindWells
...
class HCSWidget(HCSGui):
    ...
    def _plot_plate(self, list1: list, list2: list) -> None:  # to remove
        xs = [i[1] for i in list1]
        ys = [i[2] for i in list1]
        orig_xs = [i[0] for i in list2]
        orig_ys = [i[1] for i in list2]
        plt.scatter(xs, ys)
        plt.scatter(orig_xs, orig_ys)
        plt.axis("square")
        plt.show()
