...
class StardistWidget(QDialog):
    """Widget to run Stardist when core emits 'frameReady' signal."""
    ...
    def __init__(
        ...
        *,
        ...
        self._stardist_is_active = False
        ...
        self._mmc.mda.events.frameReady.connect(self._stardist_module)
    ...
    def _on_cfg_loaded(self):
        self._stardist_checkbox.setChecked(False)
    ...
    def _create_stardist_wdg(self):
        ...
        self._stardist_checkbox = QCheckBox(text="activate Stardist")
        self._stardist_checkbox.toggled.connect(self._on_stardist_checkbox_toggle)
        main_layout.addWidget(self._stardist_checkbox)
        ...
        self._stardist_channel = QComboBox()
        self._reset_channel_list()
        ch_layout.addWidget(ch_lbl)
        ch_layout.addWidget(self._stardist_channel)
    ...
    def _reset_channel_list(self):
        self._stardist_channel.clear()

        if not self._mmc.getChannelGroup():
            return

        self._stardist_channel.addItems(
            list(self._mmc.getAvailableConfigs(self._mmc.getChannelGroup()))
        )

    def _on_stardist_checkbox_toggle(self, state: bool):
        self._stardist_is_active = state
    ...
    def _disconnect(self):
        ...
        self._mmc.mda.events.frameReady.disconnect(self._stardist_module)
    ...
    def _update_mda_engine(self, newEngine: PMDAEngine, oldEngine: PMDAEngine) -> None:
        oldEngine.events.frameReady.disconnect(self._stardist_module)
        newEngine.events.frameReady.connect(self._stardist_module)
    ...
    def _get_shape_and_channels(self, sequence: MDASequence):
        ...
        if self._mda_meta.split_channels:
            ...
            channels = [f"{self._stardist_channel.currentText()}_"]

        return shape, channels

    def _create_zarr(self, sequence: MDASequence):
        if not self._stardist_is_active:
            return
        ...
        for i, channel in enumerate(channels):
            ...
            if channel[:-1] != self._stardist_channel.currentText():
                continue

            id_ = f"stardist_{channel}{sequence.uid}"
    ...
    def _run_stardist(self, image: np.ndarray, event: MDAEvent) -> Generator:
        ...
        if event.channel.config != self._stardist_channel.currentText():
            yield (event,)
    ...
    def _add_to_viewer(self, *args) -> None:
        ...
        labels_nuclei, event = args[0]

        axis_order = list(event_indices(event))

        channel = f"{self._stardist_channel.currentText()}_"
        if self._mda_meta.split_channels:
        ...
        self._mda_temp_arrays[f"stardist_{channel}{event.sequence.uid}"][
            im_idx
        ] = labels_nuclei
        self.viewer.layers[f"stardist_{channel}{event.sequence.uid}"].visible = False
        self.viewer.layers[f"stardist_{channel}{event.sequence.uid}"].visible = True
